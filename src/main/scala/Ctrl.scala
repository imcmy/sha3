...
class CtrlModule(val W: Int, val S: Int)(implicit val p: Parameters)
    extends Module
  ...
  buffer_wen := false.B //Defaut value
  val buffer_waddr = Wire(UInt(W.W))
  val buffer_wdata = Wire(UInt(W.W))
  val buffer_rdata = UInt(W.W)
  buffer_waddr := 0.U
  buffer_wdata := 0.U
  if (buffer_sram) {
    when(buffer_wen) {
  ...
  val first_pad = if (p(Sha3Keccak)) "b0000_0001".U else "b0000_0110".U
  val last_pad = "b1000_0000".U
  val both_pad = first_pad | last_pad
  //last word with message in it
  val words_filled = Mux(mindex > 0.U, mindex - 1.U, mindex)
    ...
    is(m_pad) {
      ...
      when(!(buffer_count < mindex && (pindex >= buffer_count))) {
        ...
        when(pindex > words_filled && pindex < (round_size_words - 1).U) {
          ...
          when(
            byte_offset === 0.U && (pindex === words_filled + 1.U)
              && mindex =/= 0.U
          ) {
            if (buffer_sram) {
              buffer_wen := true.B
